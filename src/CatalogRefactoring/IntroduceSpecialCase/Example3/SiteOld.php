...
class SiteOld
{
    ...
    {
        $site = CustomerOld::getData();
        return empty($site->customer) ? self::BASIC_BILLING_PLAN :   $site->customer->billing_plan;
    }
    public function getWeeksDelinquent()
    {
        $site = CustomerOld::getData();
        return empty($site->customer) ? 0 :  $site->customer->payment_history->weeks_delinquent_in_last_year;
    }
}

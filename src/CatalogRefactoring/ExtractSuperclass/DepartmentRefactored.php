...
class DepartmentRefactored extends Party
{
    public function __construct(
        ...
        $this->monthly_cost = $this->getTotalMonthlyCost();
    }
    public function getStaff()
    {
        return array_slice($this->staff, 0);
    }
    public function getTotalMonthlyCost()
    {
        $monthly_costs = array_map(function ($e) {
            return $e->monthly_costs;
        }, $this->staff);

        return array_reduce($monthly_costs, function ($sum, $cost) {
            return $sum + $cost;
        }, 0);
    }

...
class ThermostatOld
{
    private HeatingPlanOld $heatingPlan;
    public function __construct(
        protected float $current_temperature
    ) {
        $this->heatingPlan = new HeatingPlanOld((object)['selected_temperature' => $current_temperature]);
    }

    public function getTargetTemperature()
    {

        if ($this->heatingPlan->getTargetTemperature() > $this->current_temperature) {
            return $this->setToHeat();
        } elseif ($this->heatingPlan->getTargetTemperature() < $this->current_temperature) {

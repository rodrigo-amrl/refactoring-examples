...
class HeatingPlanOld
{
    ...
    public function __construct(
        protected object $thermostat
    ) {
    }
    function getTargetTemperature()
    {
        if ($this->thermostat->selected_temperature > self::MAX) {
            return self::MAX;
        } elseif ($this->thermostat->selected_temperature < self::MIN) {
            return self::MIN;
        } else {
            return $this->thermostat->selected_temperature;
        }
    }
}
